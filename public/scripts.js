...
*/
window.LAPERLA_ORDER_BACKEND_ENDPOINT = window.LAPERLA_ORDER_BACKEND_ENDPOINT || "/api/orders";
window.LAPERLA_ORDER_EMAIL_ENDPOINT = window.LAPERLA_ORDER_EMAIL_ENDPOINT || "/api/orders";
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  document.getElementById("checkoutBtn").addEventListener("click",()=>{
    if(!cart.length){toast("Ihr Warenkorb ist leer");return;}
    closeCart();
    const checkoutCart = {
      items: cart.map(x => ({ id: x.id, name: x.name, price: x.price, qty: x.qty, vol: x.vol, img: x.img })),
      total: cartTotal()
    };
    try{localStorage.setItem("laperlaCheckoutCart", JSON.stringify(checkoutCart));}catch(e){}
...
(function(){
  ...
  const API_REQUEST_ENDPOINT = "/api/b2b";
  const API_EMAIL_ENDPOINT = "/api/b2b";
  ...
  if(form) {
    form.addEventListener("submit", async function(e) {
      ...
      try {
        ...
        toast(data.success ? "✓ Anmeldung erfolgreich!" : data.message);
